...
namespace openMVG
{
  namespace sfm
  {
    ...
    // parameterized using 12 parameters: 3 for rotation, 3 for translation, 1 for
    // focal length, 2 for radial distortion and 3 for translation velocities. 
    // The principal point is not modeled. (i.e. it is assumed be located at the image center).
    struct TranslationVelocityError {
      TranslationVelocityError(double observed_x, double observed_y)
      ...
      static ceres::CostFunction* Create(const double observed_x,
        ...
        return (new ceres::AutoDiffCostFunction<TranslationVelocityError, 2, 9, 3>(
                    new TranslationVelocityError(observed_x, observed_y)));
      }
        ...
        const Optimize_Options &options)
    {
      ...
      for (const auto &pose_it : sfm_data.poses)
      {
        ...
        if (options.use_rolling_shutter_opt)
        {
          ...
          {
            std::cout << "RS : Rotation & Translation" << std::endl;
